        ...
        implements Operator
{
    ...
    private boolean advance()
    {
        ...
        while (position < sampleWeightBlock.getPositionCount()) {
            ...
            long sampleWeight = BIGINT.getLong(sampleWeightBlock, position);
            if (sampleWeight != 0) {
                remainingWeight = sampleWeight - 1;
                return true;
            }
            position++;
        }
    ...
    public Page getOutput()
    {
        while (!pageBuilder.isFull() && advance()) {
            ...
            for (int i = 0; i < blocks.length; i++) {
                types.get(i).appendTo(blocks[i], position, pageBuilder.getBlockBuilder(i));
            }
        }
