        ...
        if (object == null) {
            ...
            switch (hiveTypes[column]) {
                case SHORT:
                    ...
                    break;
                case TIMESTAMP:
                    longs[column] = ((TimestampWritable) object).getTimestamp().getTime();
                    break;
                case BYTE:
    ...
    private void parseStringColumn(int column)
    {
        ...
        switch (hiveTypes[column]) {
            ...
            case STRUCT:
                ...
                break;
            case STRING:
                Text text = Types.checkType(value, Text.class, "materialized string value");
                slices[column] = Slices.copyOf(Slices.wrappedBuffer(text.getBytes()), 0, text.getLength());
                break;
            case BINARY:
                BytesWritable bytesWritable = Types.checkType(value, BytesWritable.class, "materialized binary value");
